...
const isDateOccupied = (date: Dayjs) =>
  occupiedDates.some((occupiedDate) => occupiedDate.isSame(date, "day"));
...
const areAllSlotsOccupied = (date: Dayjs) => {
  ...
  const occupiedDay = occupiedTimes.find((occupiedTime) =>
    occupiedTime.date.isSame(date, "day")
  );
  return occupiedDay
    ? timesPerDay.every((time) =>
        occupiedDay.times.includes(time.format("HH:mm"))
      )
    : false;
};
